...
Both R and Python can plot geospatial data and have several frameworks and packages for that. R has a built-in `plot()` function that can visualise any type of data quickly, and all packages make use of it. For more advanced custom visualisation, `ggplot2` can be used. In Python, the standard plotting package is `matplotlib`. 
...
As you see, Python interprets the missing number as the first or the last element of the array, depending on it's missing before or after the `:`. R does not, and requires you to enter `1` for the first element and `length(YourArray)` (e.g. `length(Buildings)`) for the last one.

```{block, type="alert alert-success"}
> **Question 4**: Create a subset of `Buildings` with only `Gaia` and `Aurora`, using index slicing in R and Python. Hint: You may wish to use the functions `c()` for R. To vectorize a list in Python, you can use `numpy.array()`, but creating a plain list doesn’t require it :)
```
...
```{block, type="alert alert-success"}
> **Question 5**: In R and Python, slice `LocMat` to select all values below `6` and above `52`. Hint: You can combine conditions by using `&` (AND) or `|` (OR). Note: In Python, always use parentheses around each condition when combining them — otherwise, operator precedence may cause unexpected behavior.
```

## Conditionals and control flow

We just saw that we can slice arrays based on conditions. Note that the boolean operators `&` and `|` (for *and* and *or* respectively) are valid in both R and Python, but they mean different things!
...
This is good in some cases, such as slicing arrays using boolean arrays (like in Question 5, where `&&` doesn't really work), and not so good in other cases, like `if` statements that control program flow.  This is because `if` requires a single `TRUE` or `FALSE`, not an array.

The conditionals `if` and `else` are used to structure our code, so that part of the code only runs when certain conditions are met. In this case, `if (c(TRUE, FALSE))` is impossible to determine and thus will result in an error. The form `&&` only compares one boolean with another boolean, which makes sure you do not accidentally use arrays where a single boolean is needed, and thus is suitable. 
...
The `&` operator can still work in if statements when both sides are single boolean values. However, it is less strict and may lead to unexpected behavior if vectors are accidentally passed.

In Python, `&` is a *bitwise* comparison, that is, it does not work on booleans, but numbers. 
...
This behavior makes them useful in contexts like Question 5, where we compare an array of values against numeric thresholds and want element-wise boolean logic.
Instead, the regular boolean comparison is done using `and`, the logical comparison. It expects the entire variable to represent a single boolean value (True or False). That’s why `and` or `or` cannot be used for element-wise slicing in Question 5.
So just like in R, when you need a single boolean result to control program flow in `if` statement, `and` is more appropriate than `&`.

Equivalent example code in Python (note the syntax differs a bit between R and Python, e.g. the lack of parentheses, the words for `and` and `or`, and the special `elif` keyword):
...
```{block, type="alert alert-success"}
> **Question 6**: Why don't we need parentheses here in this Python code? Hint: it's something to do with operator precedence.
```
